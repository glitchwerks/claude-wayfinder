...
The fix is straightforward: remove the `verbose` entry from `triggers.keywords`. A keyword whose term no longer appears in the body is by definition a misleading signal. Stale keywords are the most common source of unexpected `ambiguous` decisions — the entry scores well on prompts that no longer reflect its purpose.

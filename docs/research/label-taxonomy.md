...
| `docs_prose` | The task produces or modifies prose documentation | `docs/**`, `*.md`, `*.rst`, README, changelog, spec files, plan files under `docs/`; verbs "write docs", "update readme", "add changelog entry" |
| `project_meta` | The task is about project coordination, planning, or VCS state — not producing a code or prose artifact | GitHub issue/PR queries or writes, project planning, reading spec/plan files to scope work, CI status checks; `docs/superpowers/specs/` and `docs/superpowers/plans/` paths signal project-level planning |
...
- **`docs/superpowers/specs/` or `docs/superpowers/plans/` paths with no code file:**
  these are project-level planning or spec documents → `project_meta`.

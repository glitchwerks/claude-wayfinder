...
| `research` | Prior-art markers from §10 E10 frozen set ("prior art", "what exists", "alternatives", "has anyone") plus the artifact — if any — plays a REFERENCE / BASELINE / CONSTRAINT role (#407 subject-vs-reference gate). **`research` MAY apply even with artifacts present** when the artifact seeds an open exploration whose deliverable is discovered options. **`research` does NOT apply** when the artifact is the SUBJECT being understood or worked — then `diagnose` branch (b) or `build` applies. The distinction is the artifact's ROLE, not its presence. (Prior wording: "no artifact-bearing evidence" gate — superseded by #407.) |
| `operate` | Non-null `command_prefix` field, or VCS-command shape in prompt (§10 E8), where the VCS/GitHub query IS the deliverable. **Incidental-read exemption (#408):** a GitHub-issue or PR read used as a MEANS to a subsequent codebase investigation (read issue to understand scope, then explore/grep/read code) does NOT fire `operate` — dominant intent is investigation → `diagnose` (branch b). `operate` fires only when querying VCS/GitHub state IS itself the end of the task. |
...
| `docs_prose` | `docs/**`, `*.md`, `*.rst`, `*.adoc`, README files, prose artifact targets. **Also:** editing the prose of an existing plan/spec/design doc (posture `build`; deliverable = modified document) → `docs_prose`. Corpus 34712 worked example: edits `docs/superpowers/plans/2026-06-04-…md` to add a slice → `docs_prose`. |
| `project_meta` | Issue/PR scope questions, project planning, VCS metadata, CI status; spec/plan file paths (`docs/superpowers/specs/`, `docs/superpowers/plans/`). **Also:** reading a plan/spec doc to scope or phase new work (posture `plan`; deliverable = new plan) → `project_meta`. **The path alone does not decide domain (#410) — the action does.** |
...
**Resolved — corpus 35414 (2026-06-19):** previously carried `disputed: true` (dispute:
...
"investigator"` — `src/baton_harness/` is the SUBJECT being investigated to design
...
| 2026-06-19 | **Three labeling-rule refinements (R1 #410, R2 #408, R3 #407).** R1 — plan-doc-edit domain: `docs_prose`/`project_meta` split for plan/spec/design doc paths now decided by ACTION not path — editing prose of a plan doc (posture `build`) → `docs_prose`; reading to scope new work (posture `plan`) → `project_meta`; §3 Step 2 `docs_prose`/`project_meta` rows and §4 domain hard-case updated; corpus 34712 added as worked example. R2 — incidental-issue-read `operate` exemption: a GitHub-issue or PR read that is a means to a subsequent codebase investigation resolves to `diagnose` (branch b), not `operate`; `operate` fires only when the VCS/GitHub query is itself the deliverable; §3 Step 1 `operate` row and §4 posture hard-case updated; corpus 35266/35297 cited. R3 — subject-vs-reference research gate: replaces "no artifact-bearing evidence" E9 gate for `research`; `research` MAY apply with artifacts present when the artifact is a reference/baseline/constraint seeding open exploration; `research` does NOT apply when the artifact is the subject being investigated (`diagnose` b) or worked (`build`); §3 Step 1 `research` row updated; corpus 35414 resolved — `src/baton_harness/` is the subject → `diagnose`/`investigator`, `disputed` flipped to `false` (§6 resolution note added). | issues #407; #408; #410 | R1: domain reclassification applies to plan/spec/design doc edits — labelers should re-examine any plan-path entry previously classified `project_meta` with posture `build`; 34712 confirmed `docs_prose`. R2: 35266/35297 already carry `diagnose` from prior adjudication — no net label change. R3: 35414 `disputed` flag flipped to `false`; all other `research` gold labels unaffected (existing research-labeled entries either have no artifacts or the artifact plays a reference role). |
